...
- fix: run `on_redirect_callback` only on redirect code exchange, not silent restore
- feat: add `on_session_restore_callback` for non-redirect authenticated restores
- fix: migrate legacy localStorage refresh token key (`kinde_refresh_token`) to chunked key format before `checkAuth`
- docs: clarify `getToken()` deprecation and migration to `getAccessToken()`
